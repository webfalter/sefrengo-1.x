	...
	function get_db($db_interface, $db_tables, $inc_path) {
		...
		if (!isset($this->_db["'$dbtype'"])) {
			...
			if (!file_exists($inc_path.'class.querybuilder_'.$dbtype.'.php')) {
				die('Querybuilder f�r Datenbank <b>'.$db_interface->Database.'</b> vom Typ <b>'. $dbtype .'</b> nicht gefunden.');
			} else {
    ...
    global $cfg_cms;
		...
		if (!isset($this->_db["'ado_$db_type'"])) {
			...
			if (!file_exists($inc_path.'adodb5/adodb.inc.php')) {
				die("phpADODB f�r Datenbank vom Typ <b>$db_type</b> nicht gefunden.");
			} else {
...
**
** Copyright   : J�rgen Br�ndle, 2002-2003
** Author      : J�rgen Br�ndle, braendle@web.de
